...
class CycleGANPatchModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        Here, discriminator will also be trained with fake labels
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_identity', type=float, default=0.05, help='use identity mapping. Setting lambda_identity other than 0 has an effect of scaling the weight of the identity mapping loss. For example, if the weight of the identity loss should be 10 times smaller than the weight of the reconstruction loss, please set lambda_identity = 0.1')
        return parser
    ...
    def backward_D_basic(self, netD, real, fake, realidx, fakeidx,):
        ...
        """
        # Real
        pred_real = netD(real, realidx)
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake (from fake images with true labels)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A = self.netG_A(self.real_B, self.real_B_patchidx)
            self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B = self.netG_B(self.real_A, self.real_A_patchidx)
