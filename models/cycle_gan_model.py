...
from .networks import masked_l1
...
class CycleGANModel(BaseModel):
    ...
    def backward_D_basic(self, netD: nn.Module, real: torch.Tensor, fake: torch.Tensor, real_mask: torch.Tensor, fake_mask: torch.Tensor):
        ...
        Parameters:
            ...
            real_mask (tensor array) -- pad mask for the real image
            fake_mask (tensor array) -- pad mask for the fake image
        ...
        loss_D_real = self.criterionGAN(pred_real, True, mask=real_mask)
        loss_D_fake = self.criterionGAN(pred_fake, False, mask=fake_mask)
    ...
    def backward_D_A(self):
        ...
        fake_B, mask_fake_B = self.fake_B_pool.query(images=self.fake_B, masks=self.pad_mask_A)
        self.loss_D_A = self.backward_D_basic(netD=self.netD_A, real=self.real_B, fake=fake_B, real_mask=self.pad_mask_B, fake_mask=mask_fake_B)

    def backward_D_B(self):
        """Calculate GAN loss for discriminator D_B"""
        fake_A, mask_fake_A = self.fake_A_pool.query(self.fake_A, masks=self.pad_mask_B)
        self.loss_D_B = self.backward_D_basic(netD=self.netD_B, real=self.real_A, fake=fake_A, real_mask=self.pad_mask_A, fake_mask=mask_fake_A)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.loss_idt_A = self.criterionIdt(pred=self.idt_A, target=self.real_B, mask=self.pad_mask_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed: ||G_B(A) - A||
            self.idt_B = self.netG_B(self.real_A)
            self.loss_idt_B = self.criterionIdt(pred=self.idt_B, target=self.real_A, mask=self.pad_mask_A) * lambda_A * lambda_idt
        else:
            self.loss_idt_A = 0
            self.loss_idt_B = 0

        # GAN loss D_A(G_A(A))
        self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_B), True, mask=self.pad_mask_A)
        # GAN loss D_B(G_B(B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True, mask=self.pad_mask_B)
