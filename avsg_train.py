...
if __name__ == '__main__':
    ...
    for i_epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i_batch, scene_data in enumerate(dataset):  # inner loop within one epoch
            ...
            if total_iters % opt.print_freq == 0:
                ...
                if opt.display_id > 0:
                    visualizer.plot_current_losses(i_epoch, float(epoch_iter) / dataset_size, losses)
