...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--init_gain', type=float, default=0.02, help='scaling factor for normal, '
        ...
        parser.add_argument('--serial_batches', action='store_true', help='if true, takes images in order to make '
                                                                          'batches, otherwise takes them randomly')
        ...
        parser.add_argument('--max_dataset_size', type=int, default=float("inf"), help='Maximum number of samples '
                                                                                       'allowed per dataset. '
                                                                                       'If the dataset directory '
                                                                                       'contains more than '
                                                                                       'max_dataset_size, only a '
                                                                                       'subset is loaded.')
        parser.add_argument('--preprocess', type=str, default='resize_and_crop', help='scaling and cropping of images '
                                                                                      'at load time '
                                                                                      '[resize_and_crop | '
                                                                                      'crop | scale_width | '
                                                                                      'scale_width_and_crop '
                                                                                      '| none | pristine]')
        parser.add_argument('--no_flip', action='store_true', help='if specified, do not flip the images for data '
                                                                   'augmentation')
        parser.add_argument('--display_winsize', type=int, default=256, help='display window size for both '
                                                                             'visdom and HTML')
        # additional parameters
        parser.add_argument('--epoch', type=str, default='latest', help='which epoch to load? set to latest to '
                                                                        'use latest cached model')
        parser.add_argument('--load_iter', type=int, default='0', help='which iteration to load? '
                                                                       'if load_iter > 0, the code will load '
                                                                       'models by iter_[load_iter]; '
                                                                       'otherwise, the code will load '
                                                                       'models by [epoch]')
