...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, num_slice, dim, K, m, T, mlp):
        ...
        self.m = m
        self.T = T
        self.num_locs = num_slice # TODO: add the new dimension of number of locations
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, keys, slice_idx):
        ...
        self.queue[:, ptr[slice_idx]:ptr[slice_idx] + batch_size, slice_idx] = keys.T
        ptr[slice_idx] = (ptr[slice_idx] + batch_size) % self.K  # move pointer
    ...
    def forward(self, slice_idx, im_q, im_k):
        """
        Input:
        ...
        negs = self.queue[:,:,slice_idx].clone().detach()
        ...
        # dequeue and enqueue
        self._dequeue_and_enqueue(k, slice_idx) # TODO: consider location for each slice in the batch
